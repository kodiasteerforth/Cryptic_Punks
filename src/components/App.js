...
// Import ABI + Config
import CrypticPunks from '../abis/CrypticPunks.json'
import config from '../config.json'

function App() {
	const [web3, setWeb3] = useState(null)
	const [crypticPunks, setCrypticPunks] = useState(null)
	...
	const loadBlockchainData = async (_web3, _account, _networkId) => {
		...
		try {
			const crypticPunks = new _web3.eth.Contract(CrypticPunks.abi, CrypticPunks.networks[_networkId].address)
			setCrypticPunks(crypticPunks)

			const maxSupply = await crypticPunks.methods.maxSupply().call()
			const totalSupply = await crypticPunks.methods.totalSupply().call()
			setSupplyAvailable(maxSupply - totalSupply)

			const allowMintingAfter = await crypticPunks.methods.allowMintingAfter().call()
			const timeDeployed = await crypticPunks.methods.timeDeployed().call()
			setRevealTime((Number(timeDeployed) + Number(allowMintingAfter)).toString() + '000')

			if (_account) {
				const ownerOf = await crypticPunks.methods.walletOfOwner(_account).call()
				setOwnerOf(ownerOf)
			} else {
				setOwnerOf([])
			}
	...
	const mintNFTHandler = async () => {
		...
		// Mint NFT
		if (crypticPunks && account) {
			setIsMinting(true)
			setIsError(false)

			await crypticPunks.methods.mint(1).send({ from: account, value: 0 })
				.on('confirmation', async () => {
					const maxSupply = await crypticPunks.methods.maxSupply().call()
					const totalSupply = await crypticPunks.methods.totalSupply().call()
					setSupplyAvailable(maxSupply - totalSupply)

					const ownerOf = await crypticPunks.methods.walletOfOwner(account).call()
					setOwnerOf(ownerOf)
				})
				.on('error', (error) => {
					window.alert(error)
				...
				<section id='welcome' className='welcome'>
					...
					<Row className='header my-3 p-3 mb-0 pb-0'>
						<Col xs={12} md={12} lg={8} xxl={8}>
							<h1>Cryptic Punks</h1>
							<p className='sub-header'>Availble on 12 / 14 / 22</p>
						</Col>
						<Col className='flex social-icons'>
							<a
								href="https://twitter.com/buggykodia"
								target='_blank'
					...
					<Row className='flex m-3'>
						...
						<Col md={5} lg={4} xl={5} xxl={4}>
							...
							<p className='text'>
							By going through the code, you'll learn how to generate NFT images, upload to IPFS, create your NFT contract, and use OpenSea!
							</p>
				...
				<section id='about' className='about'>
								...
								<div>
									...
									{ownerOf.length > 0 &&
										<p><small>View your NFT on
											<a
												href={`${openseaURL}/assets/${crypticPunks._address}/${ownerOf[0]}`}
												target='_blank'
					...
					<Row style={{ marginTop: "100px" }}>
						<Col>
							{crypticPunks &&
								<a
									href={`${explorerURL}/address/${crypticPunks._address}`}
									target='_blank'
									className='text-center'>
									{crypticPunks._address}
								</a>
							}
						</Col>
					</Row>
